...
import { obterTodos, cadastrar, atualizar, deletar } from "../../service/produto-service";
...
const ControleDeProduto = () => {
    ...
    const editarProduto = (e) => {
        setModoCadastro(false);
         const produtoEncontrato = produtos.find(p => p.id == e.target.id);
       
        // Tenho que converter a data de cadastro para uma data americana.
        produtoEncontrato.dataCadastro = aplicarMascaraDateAmericanEmDataIso(produtoEncontrato.dataCadastro);
    ...
    const excluirProduto = (e) => {
        ...
        MySwal.fire({
            ...
            if(result.isConfirmed){
                ...
                deletar(produtoEncontrato)
                .then(() =>{
                    const indice = produtos.findIndex(p => p.id == produtoEncontrato.id);
    ...
    const obterTodoOsProdutos = () => {
            ...
            .then(res => {
                const a = res
                const produtos = a.map((p) => new Produto(p));
    ...
    const cadastrarProduto = () =>{
            ...
            cadastrar(produto)
            .then(response => {
                setProdutos(arr => [...arr, new Produto(response)]);
                limparCampos();
    ...
    const atualizarProduto = () =>{
            atualizar(produto)
            .then(response => {
                atualizarProdutoNaTabela(response);
                limparCampos();
    ...
    const salvarProduto = () => {
        produto.dataCadastro = aplicarMascaraISOEmDateAmerican(produto.dataCadastro);
    ...
    return (
        <div className="container">
            <div className="row">
                ...
                <div className="col-sm-12">
                    <button className="btn_principal" onClick={adicionarProduto} data-toggle="modal" data-target="#modal-produtos" data-backdrop="static" >Adicionar</button>
                </div>
                        ...
                        <tbody>
                            {produtos.map((produto, index) => (
                                <tr key={`${produto.id}-${index}`}>
                                    ...
                                    <td>{aplicarMascaraEmDataIso(produto.dataCadastro)}</td>
                                    <td>
